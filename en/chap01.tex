...
lsql-csv '-, &1.2 &1.1'
\end{verbatim}
This will print the second (\icode{\&1.2}) and the first column (\icode{\&1.1}) of the CSV file on stdin. If you know SQL, you can read it like \icode{from stdio S select S.second, S.first}.
...
If you know SQL, you can read it as \icode{from /etc/passwd P select * where P.UID \textgreater= 1000}. As you can see, the lsql style is more compressed than standard SQL.
...
You can see here the first usage of \icode{by} block, which is equivalent to \icode{group by} in SQL.
...
\end{verbatim}
This will print all pairs of users and its group excluding the default group. If you know SQL, you can read it as \icode{from /etc/passwd P, /etc/group G select P.1, G.1 where P.1 in G.4}.
...
Some chars cannot be in symbol names (column names). For simplicity, we can suppose, they are everything but alphanumerical chars excluding \icode{-}, \icode{.}, \icode{\&}, and \icode{\_}.  
...
\begin{itemize}
    ...
    \item Bash brace expansion (e.g. \icode{{22..25} $\rightarrow$ 22 23 24 25)} \cite{bash-reference-manual}
