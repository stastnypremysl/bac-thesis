...
By running the following command it generates the build folder and \icode{lsql-csv} binary under it.
\begin{verbatim}
make
\end{verbatim}
...
primary representation of text, I would like to emphasize that in this project, \icode{String} is used as a primary
...
\begin{itemize}
    ...
    \item \icode{Lsql.Csv.Lang.From} -- This namespace contains parsers for the from block
...
\begin{itemize}
    \item \icode{Lsql.Csv.Core.BlockOps} -- This module contains the \icode{Block} definition representing a command block and functions for getting a specific type of blocks from a list of \icode{Block}.
    \item \icode{Lsql.Csv.Core.Evaluator} -- This module contains the evaluator of a \break\icode{lsql-csv} program.
    ...
    \item \icode{Lsql.Csv.Core.Tables} -- This module contains the definition of \icode{Value}, \icode{Table}, and \icode{Column}, classes over them, and functions for manipulation of them.
    \item \icode{Lsql.Csv.Lang.Args} -- A module for command line argument parsing.
    \item \icode{Lsql.Csv.Lang.BlockChain} -- This module contains the main parser of blo\-cks other than the from block.
    \item \icode{Lsql.Csv.Lang.BlockSeparator} -- This module contains the preprocessor parser, which splits a command into a list of strings---one string per one block.
    \item \icode{Lsql.Csv.Lang.Options} -- This module implements the common \icode{Option} type for from blocks and command-line optional arguments representation, and its parsers.
    ...
    \item \icode{Lsql.Csv.Main} -- This module contains the starting point for a \icode{lsql-csv} evaluation.
...
The function first calls preprocessor in \icode{Lsql.Csv.Lang.BlockSeparator}, which splits command into a list of strings. Then \icode{SymbolMap} with input is loaded using \icode{Lsql.\allowbreak Csv.Lang.From.Block} and after that
rest of the blocks are parsed using \icode{Lsql.\allowbreak Csv.\allowbreak Lang.\allowbreak BlockChain}. The program is then evaluated using
\icode{Lsql.\allowbreak Csv.Core.\allowbreak Evaluator} and finally, the output generated by \icode{Lsql.\allowbreak Csv.Utils.CsvGenerator}.
